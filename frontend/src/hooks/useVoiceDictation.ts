...
export function useVoiceDictation(
  ...
  const startRecognition = useCallback((wakeMode: boolean) => {
    const rec = recognitionRef.current;
    if (!rec) return;
    if (wakeMode && assistantActiveRef.current) return;
    if (!wakeMode && !assistantActiveRef.current) return;
  ...
  const scheduleWakeRestart = useCallback(() => {
    if (!alwaysListenRef.current || assistantActiveRef.current) return;
    clearRestartTimer();
  ...
  const endAssistantSession = useCallback(
    (options?: { notify?: boolean; recognitionAlreadyStopped?: boolean }) => {
      if (!assistantActiveRef.current) return;
  ...
  const beginAssistantSession = useCallback(
    (options?: { remainder?: string; notifyWake?: boolean }) => {
      clearRestartTimer();
      heardSpeechRef.current = false;
  ...
  useEffect(() => {
    ...
    rec.onresult = (event: SpeechRecognitionEvent) => {
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (!assistantActiveRef.current) {
          if (!alwaysListenRef.current) continue;
