...
export function findPlainRangeInRendered(
  fullText: string,
  markdown: string,
  selStart: number,
  selEnd: number
  ...
  for (let i = startInFull; i <= fullText.length; i++) {
    ...
    if (acc.length > normTarget.length + 2) break;
  }
...
function wrapCommentRange(
  ...
  try {
    ...
    if (!expanded) pop.setAttribute("hidden", "");
...
export function unwrapNoteCommentHighlights(container: HTMLElement): void {
  let found = true;
  while (found) {
    found = false;
 ...
 */
export function applyNoteCommentHighlights(
  ...
  const withSel = comments.filter(
    (c) => c.selectionStart != null && c.selectionEnd != null && c.selectionEnd > c.selectionStart!
  );
  ...
  for (const comment of sorted) {
    ...
    const plain = findPlainRangeInRendered(
      fullText,
      markdown,
