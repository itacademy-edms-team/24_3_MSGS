...
export function sanitizeFileBaseName(title) {
    const base = title.trim().replace(/[/\\?%*:|"<>]/g, "-").replace(/\s+/g, " ").slice(0, 120);
    return base || "note";
}
/** Скачать один .md-файл в браузере */
export function downloadMarkdownFile(fileBaseName, content, options) {
    const safe = sanitizeFileBaseName(fileBaseName);
    const name = options?.uniqueId != null ? `${safe}-${options.uniqueId}.md` : `${safe}.md`;
